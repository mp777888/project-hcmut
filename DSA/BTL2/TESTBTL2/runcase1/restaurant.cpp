...
public:
	...
	void printTreeHelper(HuffNode<E>* node, int indent) {
        ...
        if (node->isLeaf()) {
            ...
            solution << leaf->val() << " (" << leaf->weight() << ")";
        } else {
            IntlNode<E>* internal = dynamic_cast<IntlNode<E>*>(node);
            solution << "Internal Node (" << internal->weight() << ")";
            solution << "\n" << string(indent, ' ') << "Left: ";
	...
	void printInOrderTree(HuffNode<E> *root){
		if(root){
			...
			if(root->isLeaf()){
				...
				solution << leaf->val() << endl;
			}
			else{
				IntlNode<E>* internal = dynamic_cast<IntlNode<E>*>(root);
				solution << internal->weight() << endl;
			}
